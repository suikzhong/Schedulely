...
type UsersResponse = { users: Array<{ id: string }> }
type SpaceResponse = { space: { id: string } }
type OverlapResponse = { suggestions: Array<{ startAt: string; endAt: string }> }
...
const users = usersRes.json<UsersResponse>().users
if (users.length < 2) {
...
const spaceId = createSpaceRes.json<SpaceResponse>().space.id
...
const overlap = overlapRes.json<OverlapResponse>()
...
if (usersAfterReset.statusCode !== 200 || usersAfterReset.json<UsersResponse>().users.length < 3) {
