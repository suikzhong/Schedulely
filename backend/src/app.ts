...
import type { MeetingProposal, SharedSpace, SharedSpaceMember, User } from './types.js'
...
export async function buildApp() {
  ...
  app.get('/users/:id/availability', async (request, reply) => {
    const { id } = request.params as { id: string }
    const days = Number((request.query as { days?: string }).days ?? HORIZON_DAYS)
  ...
  app.post('/spaces', async (request, reply) => {
    ...
    const members: SharedSpaceMember[] = dedupedMemberIds.map((userId) => ({
      id: randomUUID(),
      spaceId: space.id,
      userId,
    }))
  ...
  app.get('/spaces/:id/overlap', async (request, reply) => {
    const { id } = request.params as { id: string }
    const query = request.query as { durationMin?: string; days?: string }
  ...
  app.post('/spaces/:id/proposals', async (request, reply) => {
    const { id } = request.params as { id: string }
    const body = request.body as { proposerUserId?: string; startAt?: string; endAt?: string; durationMin?: number }
